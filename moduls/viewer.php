...
$page=$TOTAL-1; // будем выводить последнюю страницу
 $diapazon=$page*9;
$sql='SELECT * FROM '.$animal.' LIMIT '.$diapazon.', 10';
 $sql_res=mysqli_query($mysqli, $sql);
 $ret=''; // строка с будущим контентом страницы
 while( $row=mysqli_fetch_assoc($sql_res) ) // пока есть записи
{
 $ret.='<div class="card text-center"><img class="card-img-top" src="data:image/jpeg;base64,'.base64_encode($row['photo']).'" /><div class="card-body"><div class="card-title">'.$row['name'].'</div><div class="card-text"><p>'.$row['blood'].',</p><p>'.$row['age'].',</p><p>'.$row['geo'].'</p> <a href="#" class="btn btn-warning">Забрать домой!</a></div></div>';
